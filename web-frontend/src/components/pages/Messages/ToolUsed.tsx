    ...
    return (
        ...
        >
            <div className={`flex justify-between items-center p-2 cursor-pointer ${isOpen ? 'border-b border-gray-600' : 'bg-transparent'}`} onClick={() => setIsOpen(!isOpen)}>
                <p>{svg_tool[tool.name] || svg_tool.default} <span className="font-semibold">{tool.name}</span>{` (${tool.call_id.slice(0, 5)}...)`}</p>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
                </svg>
            </div>
            {isOpen && (
                <div id={`tool-${tool.call_id}`} className="rounded-md mt-1 p-3 ml-4">
                    <div className="mb-2">
